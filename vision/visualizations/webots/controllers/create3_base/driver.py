...
class Driver(Supervisor):
    ...
    def initialization(
        ...
        self.image_save_dir = os.path.join("saved_images", self.world_name)
    ...
    def run(self):
        ...
        while self.step(self.timestep) != -1:
            # -------------------------------
            # 1. Get camera image and convert to RGB format
            # -------------------------------
            image = self.camera.getImage()
            width = self.camera.getWidth()
            height = self.camera.getHeight()
            # Webots returns the image in RGBA format; convert it to a numpy array
            img_array = np.frombuffer(image, np.uint8).reshape((height, width, 4))
            # Extract the RGB channels; shape is (height, width, 3)
            rgb_array = img_array[:, :, :3]

            # -------------------------------
            # 2. Call the visual model to obtain a 1280-dimensional visual feature vector
            # -------------------------------
            visual_output = get_visual_output(rgb_array)  # returns a torch.Tensor with shape (1280,)
            visual_output = visual_output.float()

            # -------------------------------
            # 3. Compute the output of the BVC layer (400 dimensions)
            # -------------------------------
            self.compute_pcn_activations()  # Update all sensor and network activations, including the BVC layer
            bvc_output = self.pcn.bvc_activations.detach().cpu()  # shape: (400,)

            # -------------------------------
            # 4. Use the fusion layer if enabled, otherwise use only BVC output
            # -------------------------------
            if self.enable_visual_fusion:
                # The fusion_layer integrates BVC (400) and visual (1280) information,
                # normalizes the result, and maps it to a 400-dimensional fused feature vector.
                fusion_features = self.fusion_layer(bvc_output, visual_output)
            else:
                fusion_features = bvc_output  # Only use BVC output

            # -------------------------------
            # 5. Pass the (fused) information to the Place Cell Layer
            # -------------------------------
            self.pcn.fusion_features = fusion_features  # The Place Cell Layer should check this attribute
            ...
            # -------------------------------
            # 6. Continue with other control logic (motion, exploration, etc.)
            # -------------------------------
            if self.robot_mode == RobotMode.MANUAL_CONTROL:
                self.manual_control()
            elif self.robot_mode in (RobotMode.LEARN_OJAS, RobotMode.LEARN_HEBB, RobotMode.DMTP, RobotMode.PLOTTING):
                self.explore()
            ...
            # Real-time print of the timestep count in the current batch
            # print(f"Time step count in current batch: {self.timestep_counter}")

            # --- For training set generation: check if the save interval has been reached ---
            # if self.timestep_counter >= self.save_interval:
            # Create a directory for the current batch, starting from batch_1
            # batch_dir = os.path.join("saved_data", f"batch_{self.batch_counter}")
            # os.makedirs(batch_dir, exist_ok=True)

            # Save image files: one image per timestep, named 1.png, 2.png, …, save_interval.png
            # for i, img in enumerate(self.image_data_list):
            # img_filename = os.path.join(batch_dir, f"{i + 1}.png")
            # cv2.imwrite(img_filename, cv2.cvtColor(img, cv2.COLOR_RGB2BGR))

            # Save BVC outputs to a CSV file: each row corresponds to one timestep's output
            # csv_filename = os.path.join(batch_dir, "bvc_outputs.csv")
            # with open(csv_filename, mode='w', newline='') as csvfile:
            # writer = csv.writer(csvfile)
            # for row in self.bvc_data_list:
            # writer.writerow(row)

            # print(
            # f"Batch {self.batch_counter} saved: {self.timestep_counter} timesteps saved to directory: {batch_dir}")

            # Reset the counter and data lists, and increment the batch counter
            # self.timestep_counter = 0
            # self.image_data_list = []
            # self.bvc_data_list = []
            # self.batch_counter += 1
