...
def plot_path(ax, hmap_x, hmap_y):
    """Plots the trajectory path over the environment."""
    # Create more points for a smoother path
    ...
    ax.plot(x_smooth, y_smooth, "r-", linewidth=1.5, alpha=0.8, label="Path")
    ax.plot(hmap_x, hmap_y, "ro", markersize=2, alpha=0.7)  # Mark each original point
...
def main():
    ...
    # Draw path on the background layer
    plot_path(ax, hmap_x, hmap_y)
    ...
    nx.draw_networkx_edges(G, pos, edge_color=edge_colors, width=edge_widths, arrows=True, arrowsize=10, ax=ax)

    # Add legend for head directions, positioned outside the plot area
    for hd in range(n_hd):
        ax.plot([], [], color=colors[hd], label=f'Head Direction {hd + 1}')
    ax.legend(title="Head Directions", bbox_to_anchor=(1.05, 1), loc='upper left')

    ax.set_title('Graph of Place Cell Connections with Head Direction Coloring and Trajectory Path')
