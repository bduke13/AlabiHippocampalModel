...
def calculate_metrics(G, positions):
    """Calculate the requested metrics for the graph."""
    ...
    # Number of feasible and infeasible edges
    feasible_edges = 0
    infeasible_edges = 0
    for u, v, data in G.edges(data=True):
        # Define a feasibility criterion, for example, edge weight above a threshold
        if data['weight'] > 0:
            feasible_edges += 1
        else:
            infeasible_edges += 1
...
def main():
    ...
    # Define threshold for edges (you may adjust this threshold based on your data)
    non_zero_weights = w_rec_tripartite_normalized[w_rec_tripartite_normalized > 0]
    threshold = np.percentile(non_zero_weights, 75) if non_zero_weights.size > 0 else 0  # 75th percentile
    ...
    for hd in range(n_hd):
        plt.plot([], [], color=colors[hd], label=f'Head Direction {hd}')
