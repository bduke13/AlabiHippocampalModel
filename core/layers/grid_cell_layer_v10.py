...
class GridCellLayer:
    ...
    - Keeps size semantics from the original v10: effective_size = size / frequency_divisor,
      used directly in the cosine triads as cos(k * x).
    ...
    def __init__(
        ...
        size_per_module = torch.empty(self.num_modules, dtype=self.dtype).uniform_(*size_range)
        ...
        # Phase-based translations: t = R(theta) K^{-1} phi, with K based on effective_size
        x_trans, y_trans = [], []
        sobol = torch.quasirandom.SobolEngine(dimension=2, scramble=True)
        for m in range(self.num_modules):
            k = (size_per_module[m] / self.frequency_divisor).item()  # effective wavenumber
            theta_deg = rot_per_module[m].item()
            theta = float(np.deg2rad(theta_deg))

            # K constructed from triad wave vectors magnitudes (effective_size)
            K = torch.tensor([[k, 0.0],
                              [k / 2.0, (np.sqrt(3.0) * k) / 2.0]], dtype=self.dtype)
    ...
    def _compute_activations_grid(self, X: np.ndarray, Y: np.ndarray) -> np.ndarray:
        ...
        for i in range(H):
            ...
            rx = c * dx + s * dy
            ry = -s * dx + c * dy
            k = (self.size_params / self.frequency_divisor).unsqueeze(0)  # [1,C]
            z1 = torch.cos(k * rx)
            z2 = torch.cos(k * (rx / 2.0 + (np.sqrt(3.0) / 2.0) * ry))
            z3 = torch.cos(k * (rx / 2.0 - (np.sqrt(3.0) / 2.0) * ry))
            z = (z1 + z2 + z3) / 3.0
    ...
    def get_grid_cell_activations(
        ...
        *,
        ...
        k = self.size_params / self.frequency_divisor
        z1 = torch.cos(k * rx)
        z2 = torch.cos(k * (rx / 2.0 + (np.sqrt(3.0) / 2.0) * ry))
        z3 = torch.cos(k * (rx / 2.0 - (np.sqrt(3.0) / 2.0) * ry))
        z = (z1 + z2 + z3) / 3.0
