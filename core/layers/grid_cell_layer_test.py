...
class GridCellLayer:
    def __init__(
        ...
        Args:
            ...
            translation_factor (float): Factor controlling translation range (-factor to +factor).
            frequency_divisor (float): Divisor to increase spacing between grid fields.
            threshold (float): Activation threshold value (0.0 to 1.0).
            threshold_type (str): Type of thresholding ('hard', 'soft', 'binary').
            sparsity (float): If provided, keep only top X% of activations.
            normalization (str): Normalization method ('none', 'global', 'per-cell', 'soft', 'local').
            local_group_size (int): Group size for local normalization.
        ...
        # Translation ranges based on translation_factor
        x_trans_range = (-translation_factor, translation_factor)
        y_trans_range = (-translation_factor, translation_factor)
        ...
        self.size_params = torch.FloatTensor(num_cells).uniform_(*size_range).to(self.device)
    ...
    def get_grid_cell_activations(self, position, threshold=None, threshold_type=None, sparsity=None, normalization=None):
        ...
        z2 = torch.cos(0.5 * effective_size * x_rot + (np.sqrt(3)/2) * effective_size * y_rot)
        z3 = torch.cos(0.5 * effective_size * x_rot - (np.sqrt(3)/2) * effective_size * y_rot)
