...
class MultiscalePlaceCellWithGrid:
    ...
    def __init__(
        ...
        # Recurrent adjacency-learning density controls (STDP path)
        adjacency_learning_mode: str = "dense",  # "dense" | "topk"
        ...
        Args:
            ...
            adjacency_learning_mode: Density mode for recurrent adjacency learning (STDP updates).
            adjacency_topk: If mode is "topk", number of active PCs retained for recurrent updates.
            adjacency_activity_floor: Optional minimum activation for a PC to contribute to recurrent updates.
        """
        ...
        self.adjacency_learning_mode = adjacency_learning_mode
        self.adjacency_topk = adjacency_topk
        self.adjacency_activity_floor = adjacency_activity_floor
        ...
        self._correlation_weights_cache = torch.ones(num_pc, num_pc, dtype=dtype, device=device)
    ...
    def _ensure_learning_caches(self) -> None:
        """Initialize cache/state fields used in learning-time hot paths."""
        if not isinstance(getattr(self, "activation_history", None), deque):
            ...
            self.activation_history = deque(self.activation_history, maxlen=self.correlation_window)

        if (
            not hasattr(self, "_correlation_weights_cache")
            or self._correlation_weights_cache.shape != (self.num_pc, self.num_pc)
    ...
    def _ensure_runtime_caches(self) -> None:
        """Initialize lightweight cached tensors used in preplay hot paths."""
        if not hasattr(self, "_turn_options") or self._turn_options.device != self.device:
            self._turn_options = torch.tensor([-1, 0, 1], dtype=torch.long, device=self.device)

        if (
            not hasattr(self, "_hd_direction_indices")
            or self._hd_direction_indices.device != self.device
            or self._hd_direction_indices.shape[0] != self.n_hd
        ):
            self._hd_direction_indices = torch.arange(self.n_hd, dtype=torch.long, device=self.device)

        if (
            not hasattr(self, "_hd_unit_vectors")
            or self._hd_unit_vectors.device != self.device
            or self._hd_unit_vectors.shape[0] != self.n_hd
        ):
            hd_angles = self._hd_direction_indices.to(dtype=self.dtype) * (2 * np.pi / self.n_hd)
            self._hd_unit_vectors = torch.stack([torch.cos(hd_angles), torch.sin(hd_angles)], dim=1)
    ...
    def get_place_cell_activations(
        ...
        if isinstance(distances, torch.Tensor):
            distances_torch = distances.to(dtype=self.dtype, device=self.device)
        else:
            distances_torch = torch.as_tensor(
        ...
        if hd_activations is not None:
            if isinstance(hd_activations, torch.Tensor):
                hd_activations_torch = hd_activations.to(dtype=self.dtype, device=self.device)
            else:
        ...
        if grid_activations is not None and self.num_grid_cells > 0:
            if isinstance(grid_activations, torch.Tensor):
                ...
                self.grid_cell_activations = torch.as_tensor(
                    grid_activations, dtype=self.dtype, device=self.device
                )
        ...
        self.activation_update.mul_(1.0 - self.tau_p)
        self.activation_update.add_(
            self.tau_p * (afferent_excitation - afferent_inhibition - recurrent_inhibition)
        )
        ...
        # Apply threshold and activation function (compatible with older torch builds).
        self.place_cell_activations.copy_(self.activation_update)
        self.place_cell_activations.relu_()
        self.place_cell_activations.tanh_()

        has_place_cell_activity = bool(self.place_cell_activations.any().item())
        ...
        if (
            ...
            if hd_activations_torch is not None:
                ...
                topk_raw = getattr(self, "adjacency_topk", None)
                try:
                    topk = int(topk_raw) if topk_raw is not None else None
                except (TypeError, ValueError):
                    topk = None

                # Prepare per-direction learning-rate scaling once.
                if self.enable_adaptive_stdp:
                    ...
                    if self.strength_update_counter % self.strength_update_frequency == 0:
                        self.connection_strength_cache.copy_(
                            self.compute_connection_strengths(self.w_rec_tripartite)
                        )
                    lr_scale = self.get_adaptive_learning_rates(self.connection_strength_cache).view(self.n_hd, 1, 1)
                else:
                    lr_scale = self.eta_stdp

                active_mask = self.place_cell_activations > floor if floor > 0.0 else self.place_cell_activations > 0

                if mode == "topk":
                    active_indices = torch.nonzero(active_mask, as_tuple=False).flatten()
                    if active_indices.numel() > 0 and topk is not None and topk > 0:
                        k = min(int(topk), int(active_indices.numel()))
                        if k < int(active_indices.numel()):
                            active_scores = self.place_cell_activations[active_indices]
                            keep_rel = torch.topk(active_scores, k=k).indices
                            sel_indices = active_indices[keep_rel]
                        else:
                            sel_indices = active_indices

                        # True sparse recurrent adjacency update: build only KxK local delta.
                        pc_act_sel = self.place_cell_activations[sel_indices]
                        pc_trace_sel = self.place_cell_trace[sel_indices]
                        delta_small = torch.ger(pc_act_sel, pc_trace_sel) - torch.ger(pc_trace_sel, pc_act_sel)
                        update_small = hd_contrib * delta_small.unsqueeze(0)  # (n_hd, k, k)

                        if self.enable_correlation_weighting:
                            correlation_weights = self.get_correlation_weights()
                            corr_small = correlation_weights.index_select(0, sel_indices).index_select(1, sel_indices)
                            update_small = update_small * corr_small.unsqueeze(0)

                        scaled_small = update_small * lr_scale if self.enable_adaptive_stdp else self.eta_stdp * update_small

                        row_idx = sel_indices.unsqueeze(1).expand(-1, sel_indices.numel())
                        col_idx = sel_indices.unsqueeze(0).expand(sel_indices.numel(), -1)
                        self.w_rec_tripartite[:, row_idx, col_idx] += scaled_small.type(self.dtype)
                else:
                    # Dense mode with optional floor mask.
                    pc_act_for_adj = self.place_cell_activations
                    ...
                    scaled_update_rec = (
                        weighted_update_rec * lr_scale if self.enable_adaptive_stdp else self.eta_stdp * weighted_update_rec
                    )
                    self.w_rec_tripartite += scaled_update_rec.type(self.dtype)

        # Check Oja's rule for input weights if enabled
        if self.enable_ojas and has_place_cell_activity:
            # (num_pc, 1)
    ...
    def update_correlation_tracking(self, pc_activations):
        ...
        thresholded_activations = pc_activations * (pc_activations > self.correlation_threshold)
    ...
    def get_correlation_weights(self):
        ...
        self._correlation_weights_cache = weights
    ...
    def apply_correlation_weighting_to_stdp(self, connection_update):
        ...
        # Apply weights to all head direction slices via broadcast.
        return connection_update * correlation_weights.unsqueeze(0)
    ...
    def compute_connection_strengths(self, direction_connections_batch: torch.Tensor) -> torch.Tensor:
        """Vectorized connection-strength computation for all HD slices.

        Args:
            direction_connections_batch: (n_hd, num_pc, num_pc) recurrent weights.

        Returns:
            Tensor of shape (n_hd,) with per-direction connection strengths.
        """
        ...
        return torch.where(
            ...
            significant_sums / torch.clamp(significant_counts.to(self.dtype), min=1.0),
    ...
    def get_adaptive_learning_rates(self, connection_strengths: torch.Tensor) -> torch.Tensor:
        """Vectorized adaptive learning rates for all HD slices."""
        if not self.enable_adaptive_stdp:
            return torch.full(
                (self.n_hd,),
                fill_value=self.stdp_learning_rate,
                dtype=self.dtype,
                device=self.device,
            )

        decay_factor = torch.exp(-connection_strengths * self.adaptive_decay_rate)
        return self.adaptive_final_lr + (self.adaptive_initial_lr - self.adaptive_final_lr) * decay_factor
    ...
    def hierarchical_multiscale_preplay_sampling(
        ...
        for scale_idx, (scale_name, pcn, rcn) in enumerate(scales_data):
            ...
            # Shape: (batch_size,)
            initial_directions = self._hd_direction_indices.repeat_interleave(num_samples)
            ...
            for step in range(num_steps):
                ...
                # Look up precomputed unit vectors for each discrete heading.
                step_vectors = self._hd_unit_vectors[current_directions]
