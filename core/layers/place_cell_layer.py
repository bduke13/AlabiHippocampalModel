...
class PlaceCellLayer:
    ...
    def __init__(
        ...
        # Fusion features (if provided externally)
        self.fusion_features: Optional[torch.Tensor] = None
    ...
    def get_place_cell_activations(
        ...
        # Determine input features: either fused visual-BVC features or pure BVC activations
        if self.fusion_features is not None:
            # Use precomputed fusion features
            bvc_input = self.fusion_features.to(device=self.device, dtype=self.dtype)
            # Record for compatibility
            self.bvc_activations = bvc_input
        else:
            ...
            bvc_input = self.bvc_activations
        ...
        afferent_excitation = torch.matmul(self.w_in, bvc_input)

        # Compute total BVC activity for afferent inhibition
        # Afferent inhibition term: Γ^{pb} ∑_j v_j^b (Equation 3.2a)
        afferent_inhibition = self.gamma_pb * torch.sum(bvc_input)
