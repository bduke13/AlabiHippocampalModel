...
class RewardCellLayer:
    def __init__(self, num_place_cells=200, num_replay=3, learning_rate=0.1, device="cpu"):
        """
        ...
        self.w_in = torch.randn((1, num_place_cells), dtype=torch.float32, device=self.device) * 0.01
    ...
    def update_reward_cell_activations(self, input_data, visit=False):
        ...
        safe_denominator = torch.max(input_norm, torch.tensor(1e-4, dtype=torch.float32, device=self.device))
    ...
    def replay(self, pcn):
        ...
        place_cell_activations = pcn_copy.place_cell_activations.to(self.device)  # shape: (num_pc,)
        ...
        for time_step in range(20):  # ⬅ FIX: Use 20 iterations instead of 10
            ...
            norm_val = torch.sqrt(torch.max(torch.sum(place_cell_activations ** 2),
            ...
            weight_update = weight_update + exponential_decay_factor * normalized_place_cell_activations
