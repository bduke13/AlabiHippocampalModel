...
class BoundaryVectorCellLayer:
    ...
    def __init__(
        ...
        enable_prox_mod: bool = False,
        prox_gain_mode: str = "auto",
        ...
        self.dtype = dtype
        self.n_res = n_res
        self.scale_name = scale_name.lower() if scale_name is not None else None
        self.enable_prox_mod = enable_prox_mod
        self.prox_gain_mode = prox_gain_mode.lower() if prox_gain_mode is not None else "auto"
        ...
        self.max_dist = torch.tensor(max_dist, dtype=dtype, device=device)
        ...
        self.prox_gain = torch.tensor(1.0, dtype=dtype, device=device)
        self.last_min_lidar_distance = torch.tensor(0.0, dtype=dtype, device=device)

    def _resolve_prox_profile(self) -> str:
        profile = self.prox_gain_mode
        if profile != "auto":
            return profile
        if self.scale_name in {"small", "medium", "large", "xlarge"}:
            return self.scale_name
        return "small"

    def _update_prox_gain(self, distances: torch.Tensor) -> torch.Tensor:
        """Compute a scalar proximity gain from the minimum LiDAR distance."""
        if not self.enable_prox_mod:
            self.prox_gain = torch.tensor(1.0, dtype=self.dtype, device=self.device)
            self.last_min_lidar_distance = torch.nan_to_num(
                torch.min(distances), nan=self.max_dist.item(), posinf=self.max_dist.item(), neginf=0.0
            )
            return self.prox_gain

        clean_distances = torch.nan_to_num(
            distances,
            nan=self.max_dist.item(),
            posinf=self.max_dist.item(),
            neginf=0.0,
        ).to(dtype=self.dtype, device=self.device)
        min_dist = torch.min(clean_distances)
        profile = self._resolve_prox_profile()

        if profile == "medium":
            center = self.sigma_r
            gain = torch.exp(-0.5 * ((min_dist - center) / self.sigma_r) ** 2)
        elif profile in {"large", "xlarge"}:
            center = torch.zeros((), dtype=self.dtype, device=self.device)
            gain = 1.0 - torch.exp(-0.5 * ((min_dist - center) / self.sigma_r) ** 2)
        else:
            center = torch.zeros((), dtype=self.dtype, device=self.device)
            gain = torch.exp(-0.5 * ((min_dist - center) / self.sigma_r) ** 2)

        self.last_min_lidar_distance = min_dist
        self.prox_gain = torch.clamp(gain, 0.0, 1.0)
        return self.prox_gain
    ...
    def get_bvc_activation(self, distances: torch.Tensor) -> torch.Tensor:
        ...
        self._update_prox_gain(distances)
