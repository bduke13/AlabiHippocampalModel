...
class MultiscalePlaceCellWithGridV8:
    ...
    def multi_step_preplay_straight(self, max_steps: int = 2, debug: bool = False) -> tuple:
        """
        Multi-step preplay with same direction continuation.
        
        ALGORITHM:
        1. For each of 8 directions (0°, 45°, 90°, 135°, 180°, 225°, 270°, 315°)
        2. Continue straight in that direction for max_steps
        3. At each step, evaluate reward and add to cumulative total
        4. Return direction with highest total reward
        
        COMPUTATIONAL COST: Always 8 evaluations (regardless of max_steps)
        
        REWARD COMBINATION: Sums rewards across all steps
        ...
        Returns:
            tuple: (best_path, best_total_reward, all_paths_evaluated)
        """
        
        if debug:
            print(f"\n=== MULTI-STEP STRAIGHT PREPLAY DEBUG (max_steps={max_steps}) ===")
            print("Direction mapping verification:")
            for d in range(self.n_hd):
                angle_deg = d * 45  # Assuming 8 directions, 45° apart
                print(f"  Direction {d}: {angle_deg}° (?)")
            print("Note: Actual compass mapping may differ - check driver output!")
        
        current_activations = self.place_cell_activations.clone()
        all_paths_evaluated = []
        
        if debug:
            print(f"Starting evaluation with current place cell activation sum: {current_activations.sum().item():.3f}")
            print(f"Expected evaluations for {max_steps} steps: {self.n_hd} (same as 1-step!)")
        
        # Try each of the 8 initial directions
        for direction in range(self.n_hd):
            if debug:
                print(f"  Evaluating straight path in direction {direction}({direction*45}°) for {max_steps} steps")
            
            # Continue straight in this direction for max_steps
            activations = current_activations.clone()
            path = [direction] * max_steps  # All same direction
            
            total_reward = 0.0  # Sum up rewards across all steps
            
            for step in range(max_steps):
                # Take one more step in the same direction
                activations = self.preplay_from_state(activations, direction, num_steps=1)
                step_reward = self._evaluate_activations_for_reward(activations)
                
                # Add this step's reward to the total
                total_reward += step_reward
                
                if debug and max_steps <= 3:  # Only debug shorter paths to avoid spam
                    print(f"    Step {step+1}: reward={step_reward:.4f}, cumulative={total_reward:.4f}")
            
            # Store the path with total summed reward
            final_path = [direction] * max_steps
            all_paths_evaluated.append((final_path, total_reward))
            
            if debug:
                print(f"    Total summed reward: {total_reward:.4f}")
        
        # Find the best direction
        if not all_paths_evaluated:
            if debug:
                print("ERROR: No paths were evaluated!")
            return [], 0.0, []
        
        best_path, best_reward = max(all_paths_evaluated, key=lambda x: x[1])
        
        if debug:
            print(f"\nResults:")
            print(f"  Total paths evaluated: {len(all_paths_evaluated)}")
            print(f"  Best direction: {best_path[0]}({best_path[0]*45}°) for {len(best_path)} steps")
            print(f"  Best total reward: {best_reward:.4f}")
            
            # Show top 3 directions
            sorted_paths = sorted(all_paths_evaluated, key=lambda x: x[1], reverse=True)[:3]
            print("  Top 3 directions:")
            for i, (path, reward) in enumerate(sorted_paths):
                direction = path[0]
                print(f"    {i+1}. Dir {direction}({direction*45}°) × {len(path)} steps: total={reward:.4f}")
        
        return best_path, best_reward, all_paths_evaluated

    def multi_step_preplay_limited_branching(self, max_steps: int = 2, debug: bool = False) -> tuple:
        """
        Multi-step preplay with limited branching strategy.
        
        ALGORITHM:
        1. Step 1: Try all 8 directions
        2. Step 2+: For each previous direction, try 3 options:
        - Continue straight (same direction)
        - Turn counter-clockwise (-1 direction index)
        - Turn clockwise (+1 direction index)
        3. At each step, evaluate reward and add to cumulative total
        4. Return path with highest total reward
        
        COMPUTATIONAL COST: 8 × 3^(max_steps-1) evaluations
        - 1 step: 8 evaluations
        - 2 steps: 8 × 3 = 24 evaluations
        - 3 steps: 8 × 9 = 72 evaluations
        
        REWARD COMBINATION: Sums rewards across all steps
        
        Args:
            max_steps: Number of steps to look ahead (1, 2, or 3 recommended)
            debug: Whether to print detailed debug information
        ...
        if debug:
            print(f"\n=== MULTI-STEP BRANCHING PREPLAY DEBUG (max_steps={max_steps}) ===")
            print("Direction mapping verification:")
            for d in range(self.n_hd):
                angle_deg = d * 45  # Assuming 8 directions, 45° apart
                print(f"  Direction {d}: {angle_deg}° (?)")
        ...
        def evaluate_path_recursive(activations, remaining_steps, path_so_far, depth=0, accumulated_reward=0.0):
            """Recursively evaluate paths with limited branching, summing rewards at each step."""
            
            # Evaluate reward at current state and add to accumulated total
            current_step_reward = self._evaluate_activations_for_reward(activations)
            total_reward_so_far = accumulated_reward + current_step_reward
            
            if remaining_steps == 0:
                # Base case: no more steps to evaluate, return total accumulated reward
                all_paths_evaluated.append((path_so_far.copy(), total_reward_so_far))
                
                if debug and len(path_so_far) <= 2:  # Only debug shorter paths to avoid spam
                    path_str = " → ".join([f"{d}({d*45}°)" for d in path_so_far])
                    print(f"  {'  ' * depth}Path {path_str}: total_reward={total_reward_so_far:.4f}")
                
                return total_reward_so_far
            
            # Determine directions to explore at this step
            if len(path_so_far) == 0:
                # First step: try all 8 directions
                directions_to_try = list(range(self.n_hd))
                if debug:
                    print(f"  {'  ' * depth}Step 1: Trying all {self.n_hd} directions (current reward: {current_step_reward:.4f})")
            else:
                ...
                if debug:
                    dir_names = [f"{d}({d*45}°)" for d in directions_to_try]
                    print(f"  {'  ' * depth}Step {len(path_so_far)+1}: From direction {last_direction}({last_direction*45}°) trying {dir_names} (step reward: {current_step_reward:.4f}, cumulative: {total_reward_so_far:.4f})")
            ...
            for direction in directions_to_try:
                ...
                # Recursively evaluate the rest of the path with accumulated reward
                new_path = path_so_far + [direction]
                reward = evaluate_path_recursive(next_activations, remaining_steps - 1, new_path, depth + 1, total_reward_so_far)
        ...
        if debug:
            ...
            if max_steps == 1:
                expected_evaluations = self.n_hd
            else:
                expected_evaluations = self.n_hd * (3 ** (max_steps - 1))
            print(f"Expected evaluations for {max_steps} steps: {expected_evaluations}")
        
        evaluate_path_recursive(current_activations, max_steps, [], 0, 0.0)
        ...
        if debug:
            print(f"\nResults:")
            print(f"  Total paths evaluated: {len(all_paths_evaluated)}")
            print(f"  Best path: {' → '.join([f'{d}({d*45}°)' for d in best_path])}")
            print(f"  Best total reward: {best_reward:.4f}")
            ...
            print("  Top 3 paths:")
            for i, (path, reward) in enumerate(sorted_paths):
                path_str = " → ".join([f"{d}({d*45}°)" for d in path])
                print(f"    {i+1}. {path_str}: total={reward:.4f}")
        
        return best_path, best_reward, all_paths_evaluated

    def multi_step_preplay_constrained(self, forced_first_direction: int, max_steps: int = 3, debug: bool = False) -> float:
        """
        Multi-step preplay constrained to start in a specific direction.
        ...
        Returns:
            float: Best total reward for paths starting in the forced direction
        """
        if debug:
            print(f"    Constrained preplay: forced_dir={forced_first_direction}({forced_first_direction*45}°), steps={max_steps}")
        
        current_activations = self.place_cell_activations.clone()
        all_paths_evaluated = []
        
        def evaluate_path_recursive(activations, remaining_steps, path_so_far, depth=0, accumulated_reward=0.0):
            """Recursively evaluate paths with forced first direction."""
        ...
        if debug:
            print(f"    Constrained result: best_reward={best_reward:.4f}")
