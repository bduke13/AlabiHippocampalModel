...
class PlaceCellLayer:
    ...
    def __init__(
        ...
        enable_cross_scale_inhibition: bool = False,
        cross_scale_inhibition_strength: float = 0.0,
    ):
        ...
        self.enable_cross_scale_inhibition = enable_cross_scale_inhibition
        self.cross_scale_inhibition_strength = cross_scale_inhibition_strength
    ...
    def get_place_cell_activations(
        ...
        collided: bool = False,
        cross_scale_inhibition: Optional[torch.Tensor] = None,
    ):
        ...
        if self.enable_prox_mod and hasattr(self.bvc_layer, "prox_gain"):
            self.afferent_excitation = self.afferent_excitation * self.bvc_layer.prox_gain.to(
        ...
        if self.enable_cross_scale_inhibition and cross_scale_inhibition is not None:
            if isinstance(cross_scale_inhibition, torch.Tensor):
                cross_scale_term = cross_scale_inhibition.to(dtype=self.dtype, device=self.device)
            else:
                cross_scale_term = torch.tensor(
                    cross_scale_inhibition, dtype=self.dtype, device=self.device
                )
            cross_scale_term = self.cross_scale_inhibition_strength * cross_scale_term
