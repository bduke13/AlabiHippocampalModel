...
from msg_driver_v21 import MultiscaleDriverWithGrid
...
def _run_random_spawn_trials(mode, trial_config, **kwargs):
    ...
    # Execute trials using single-trial pattern (like EXPLOIT_LOCATIONS)
    successful_trials = 0
...
SCALES_DEFS = {
    ...
    "medium": {
        ...
        # STDP learning rate (moderate for medium scale)
        "stdp_learning_rate": 0.05,  # Balanced learning rate for medium resolution
        "tau_hd": .5,  # HD eligibility trace time constant
        # Adaptive learning parameters for medium scale
        "enable_adaptive_stdp": True,
        "adaptive_initial_lr": 0.10,  # High initial learning rate
        "adaptive_final_lr": 0.03,    # Low final learning rate
    ...
    "large": {
        ...
        # STDP learning rate (faster for large scale)
        "stdp_learning_rate": 0.05,  # Higher learning rate for sparse, coarse activations
        "tau_hd": .5,  # HD eligibility trace time constant
        # Adaptive learning parameters for large scale
        "enable_adaptive_stdp": True,
        "adaptive_initial_lr": 0.10,  # High initial learning rate
        "adaptive_final_lr": 0.03,    # Low final learning rate
    ...
    "xlarge": {
        ...
        # STDP learning rate (fastest for xlarge scale)
        "stdp_learning_rate": 0.05,  # Highest learning rate for very sparse activations
        "tau_hd": .5,  # HD eligibility trace time constant
        # Adaptive learning parameters for xlarge scale
        "enable_adaptive_stdp": True,
        "adaptive_initial_lr": 0.10,  # High initial learning rate
        "adaptive_final_lr": 0.03,    # Low final learning rate
...
if __name__ == "__main__":
    ...
    MODES_MAP = {
        "OJAS": RobotMode.LEARNING, # Only place field formation
        "HEBB": RobotMode.LEARNING, # Only learning connections between place cells
        "LEARNING": RobotMode.LEARNING, # Both OJAS & HEBB
        "DMTP": RobotMode.DMTP, # Once the robot get to the goal a reward map is formed
        "DMTP_EXPLOIT": RobotMode.DMTP, # DMTP with LEARNING for a specific experiment in Beks work
        "EXPLOIT": RobotMode.EXPLOIT, # Only Exxploitaiton
        "EXPLOIT_SAVE": RobotMode.EXPLOIT, # Bek Experiments
        "LEARNING_SAVE": RobotMode.EXPLOIT, # Bek Experiments
        "PLOTTING": RobotMode.PLOTTING, # Get visualations.
        "LEARN_LOCATIONS": RobotMode.LEARN_LOCATIONS, # LEARNING + Reward map for multiple goals
        "LEARN_LOCATIONS_COVERAGE": RobotMode.LEARN_LOCATIONS_COVERAGE, # LEARNING + Coverage-based termination
        "EXPLOIT_LOCATIONS": RobotMode.EXPLOIT_LOCATIONS, # Exploitation, specify starts and goals
        "EXPLOIT_LOCATIONS_RANDOM": RobotMode.EXPLOIT_LOCATIONS_RANDOM  # Exploitation with random spawns
    }
    ...
    # Coverage parameters for LEARN_LOCATIONS_COVERAGE mode
    environment_size = [20, 20]  # Environment size in meters [width, height]
    grid_size = 0.25  # Grid cell size in meters
    coverage_percentage = 0.9  # Required coverage percentage (0.0-1.0)

    # Random spawn parameters for EXPLOIT_LOCATIONS_RANDOM mode
    trials_per_goal = 20  # Number of trials per goal
    min_spawn_distance = 6  # Minimum distance from goal (meters)
    wall_clearance = 0.5  # Wall clearance factor (meters)
    path_failure_ratio = 10  # Distance multiplier for stopping condition
    ...
    MODE_PARAMS = {
        ...
        "LEARN_LOCATIONS_COVERAGE": {
            ...
            "enable_ojas": True,
            "enable_stdp": True,
            "run_time_hours": 5.0,  # High time limit as fallback
            "max_dist": max_dist,
            "plot_bvc": plot_bvc,
            "clear_files": False,
            "action_mode": 'explore',
