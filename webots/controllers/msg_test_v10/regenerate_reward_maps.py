...
def regenerate_reward_maps(association_data, pcns, rcns, output_dir):
    ...
    for scale_idx, config in SCALE_SPECIFIC_CONFIG.items():
        print(f"  Scale {scale_idx}: timesteps={config['custom_replay_timesteps']}, "
              f"decay={config['replay_decay_factor']}")
    ...
    for goal in goals:
        ...
        for scale_idx, (pcn, rcn) in enumerate(zip(pcns, rcns)):
            ...
            goal_rcn = RewardCellLayerTest(
                ...
                replay_decay_factor=config.get("replay_decay_factor", 6),
                custom_replay_timesteps=config.get("custom_replay_timesteps", 40),
                device=device,
            )

            print(f"  Replay config: timesteps={goal_rcn.custom_replay_timesteps}, "
                  f"decay={goal_rcn.replay_decay_factor}")
