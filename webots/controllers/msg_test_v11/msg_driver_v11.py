...
from core.layers.multiscale_pcn_with_gcn_v11 import MultiscalePlaceCellWithGrid
from core.layers.grid_cell_layer_v11 import GridCellLayer
...
class Driver(Supervisor):
    ...
    def exploit_v11(self):
            ...
            if use_scale_reliability and self.rotation_loop_count >= loop_threshold_reliability:
                if torch.sum(self.loop_scale_contributions) > 0:
                    for scale_idx in range(len(self.scales)):
                        penalty = reliability_decay_bad * self.loop_scale_contributions[scale_idx]
                        self.scale_reliability[scale_idx] = torch.clamp(
                            self.scale_reliability[scale_idx] - penalty,
        ...
        (final_direction_deg, expected_value, combined_vector,
         discounted_returns, direction_vectors, joint_probs, trajectory_metadata, scale_weights) = self.pcns[0].hierarchical_multiscale_preplay(
            ...
            reliability_term_factor=reliability_term_factor,
            debug=debug_enabled
        )
    ...
    def _update_scale_reliability_bad(self, scale_weights, decay_bad, attribution='weighted', min_floor=0.1,
        ...
        Args:
            ...
            decay_bad: Penalty amount (e.g., 0.05)
        ...
        if attribution == 'weighted':
            ...
            self.scale_reliability[dominant_idx] = torch.clamp(
                self.scale_reliability[dominant_idx] - decay_bad,
                min=min_floor,
                max=1.0
            )
    ...
    def _update_scale_reliability_good(self, scale_weights, decay_good, attribution='weighted', cooldown=None):
        ...
        Args:
            scale_weights: Current scale weights [num_scales] from last decision
            decay_good: Reward amount (e.g., 0.01)
            attribution: 'weighted' (proportional) or 'dominant' (argmax only)
        """
        if self.scale_reliability is None:
            return
        ...
        if attribution == 'weighted':
            ...
            for scale_idx in range(len(self.scale_reliability)):
                ...
                self.scale_reliability[scale_idx] = torch.clamp(
                    self.scale_reliability[scale_idx] + reward,
                    min=0.0,
                    max=1.0
                )
        elif attribution == 'dominant':
