...
from visual_output import get_visual_output

PROJECT_ROOT = Path(__file__).resolve().parents[3]  # Moves two levels up
sys.path.append(str(PROJECT_ROOT))  # Add project root to sys.path
# Root of visualization directory (assumed to be the 'visualizations' folder at project root)
...
class Driver(Supervisor):
    ...
    def initialization(
        ...
        # Determine world name and create directories
        if world_name is None:
            world_path = self.getWorldPath()
            world_name = os.path.splitext(os.path.basename(world_path))[0]
        self.world_name = world_name
        # Save pickle files under the visualization folder
        self.hmap_dir = str(VIS_ROOT / "pkl" / self.world_name / "hmaps")
        self.network_dir = str(VIS_ROOT / "pkl" / self.world_name / "networks")
        ...
        self.n_hd = 8
        self.timestep = 32 * 3
        # time constant for exploration window
        self.tau_w = 5
        self.max_speed = 16
        ...
        self.enable_visual_fusion = enable_visual_fusion
        ...
        # Randomize or set start location
        if randomize_start_loc:
            while True:
                INITIAL = [random.uniform(-2.3, 2.3), 0, random.uniform(-2.3, 2.3)]
                dist_to_goal = np.hypot(
                    INITIAL[0] - self.goal_location[0],
                    INITIAL[2] - self.goal_location[1]
                )
                if dist_to_goal >= 1.0:
                    break
        ...
        self.range_finder = self.getDevice("range-finder"); self.range_finder.enable(self.timestep)
        self.left_bumper = self.getDevice("bumper_left"); self.left_bumper.enable(self.timestep)
        self.right_bumper = self.getDevice("bumper_right"); self.right_bumper.enable(self.timestep)
        self.collided = torch.zeros(2, dtype=torch.int32)
        self.left_motor = self.getDevice("left wheel motor")
        self.right_motor = self.getDevice("right wheel motor")
        self.left_position_sensor = self.getDevice("left wheel sensor"); self.left_position_sensor.enable(self.timestep)
        self.right_position_sensor = self.getDevice("right wheel sensor"); self.right_position_sensor.enable(self.timestep)
        self.camera = self.getDevice("camera"); self.camera.enable(self.timestep)

        # Fusion layer
        self.fusion_layer = FusionLayer("encoder_weights.pth")

        # Boundary vector cell resolution
        self.lidar_resolution = 720
        self.boundaries = torch.zeros((self.lidar_resolution, 1), device=self.device)

        # Clear existing networks if starting fresh
        if self.robot_mode == RobotMode.LEARN_OJAS:
            self.clear()

        # Load or initialize PCN and RCN
        self.load_pcn(
        ...
        self.head_direction_layer = HeadDirectionLayer(num_cells=self.n_hd, device=torch.device("cpu"))

        # Initialize history maps
        self.hmap_loc = np.zeros((self.num_steps, 3))
        self.hmap_pcn = torch.zeros((self.num_steps, self.pcn.num_pc), device=self.device)
        self.hmap_bvc = torch.zeros((self.num_steps, self.pcn.bvc_layer.num_bvc), device=self.device)
        self.hmap_hdn = torch.zeros((self.num_steps, self.n_hd), device="cpu")
        self.directional_reward_estimates = torch.zeros(self.n_hd, device=self.device)

        # Prime simulation
        self.step(self.timestep); self.step_count += 1
        self.sense(); self.compute_pcn_activations()

    def sense(self):
        """
        1) step simulation
        2) update LiDAR boundaries, heading, HD activations, collision
        3) capture camera image & extract visual features
        """
        self.step(self.timestep)

        # LiDAR, heading, collision
        ...
        theta = np.deg2rad(self.current_heading_deg)
        v_in = torch.tensor([np.cos(theta), np.sin(theta)], dtype=self.dtype, device=self.device)
        ...
        img = np.frombuffer(image, np.uint8).reshape((h, w, 4))[:, :, :3]
        self.rgb_array = img
        vo = get_visual_output(img)
        self.visual_output = vo.float().to(self.device)

    def compute_pcn_activations(self):
        """
        1) compute BVC activation
        2) optionally plot BVC
        3) fuse with visual features
        4) update PlaceCell activations
        5) record to history maps
        """
        # BVC
        bvc_output = self.pcn.bvc_layer.get_bvc_activation(self.boundaries)
        if self.show_bvc_activation:
            self.pcn.bvc_layer.plot_activation(self.boundaries.cpu())

        # Fusion
        if self.enable_visual_fusion:
            fused = self.fusion_layer(bvc_output.detach().cpu(), self.visual_output.detach().cpu())
        else:
            fused = bvc_output.detach().cpu()

        # PlaceCell
        self.pcn.fusion_features = fused.to(self.device)
    ...
    def run(self):
        """Main loop: sense → compute activations → act."""
