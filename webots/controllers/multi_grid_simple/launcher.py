...
CONTROLLER_WORLD_ROOT = CONTROLLER_DIR / "worlds"
GENERATED_WORLD_ROOT = WORLD_ROOT
GENERATED_WORLD_PATTERNS = (
    "*_automation.wbt",
    "*_smoke.wbt",
    "*_session_smoke.wbt",
    "*_validation.wbt",
...
GENERATED_WORLD_HEADER = "# Generated by multi_grid_simple launcher"
...
def resolve_world_path(world_name_or_path: str) -> Path:
    ...
    controller_world_path = CONTROLLER_WORLD_ROOT / world_name_or_path
    if controller_world_path.exists():
        return controller_world_path
...
def canonical_world_name(world_name_or_path: str | Path) -> str:
    return resolve_world_path(str(world_name_or_path)).stem
...
def prepare_controller_world(
    ...
    *,
    ...
    if not lines:
        raise ValueError(f"World file is empty: {source_path}")
    generated_text = "\n".join(
        [
            lines[0],
            f"{GENERATED_WORLD_HEADER} from {source_path.as_posix()}",
            *lines[1:],
        ]
    ) + "\n"
...
def cleanup_generated_world(path: str | Path) -> None:
    world_path = Path(path)
    if not world_path.exists():
        return
    try:
        if world_path.resolve().parent != GENERATED_WORLD_ROOT.resolve():
            return
    except FileNotFoundError:
        return

    try:
        with world_path.open("r", encoding="utf-8") as input_file:
    ...
    matches_generated_header = (
        first_line.startswith(GENERATED_WORLD_HEADER)
        or second_line.startswith(GENERATED_WORLD_HEADER)
    )
    matches_generated_name = any(
        fnmatch(world_path.name, pattern) for pattern in GENERATED_WORLD_PATTERNS
    )
    if matches_generated_header and matches_generated_name:
        world_path.unlink(missing_ok=True)


def cleanup_stale_generated_worlds() -> list[Path]:
    removed: list[Path] = []
    GENERATED_WORLD_ROOT.mkdir(parents=True, exist_ok=True)
    for pattern in GENERATED_WORLD_PATTERNS:
        for path in GENERATED_WORLD_ROOT.glob(pattern):
            cleanup_generated_world(path)
            if not path.exists():
                removed.append(path)
    return removed
...
def launch_from_config(
    *,
    ...
    source_world_name = canonical_world_name(launch_config["world"])
    world_path = prepare_controller_world(
        launch_config["world"],
        controller_name=controller_name,
    )
    launch_env = dict(env or {})
    launch_env.setdefault("MULTI_GRID_SIMPLE_CANONICAL_WORLD_NAME", source_world_name)
