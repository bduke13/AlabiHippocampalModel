...
from core.layers.msg_pcn_v10 import MultiscalePlaceCellWithGridV8
...
class MultiscaleDriverWithGrid(Supervisor):
    def initialization(
        ...
        self.STEP_DECAY_FACTOR = 1.0
        self.action_mode = action_mode
        self.DEBUG_EXPLOIT_V0 = True
        self.DEBUG_EXPLOIT_V1 = False

        self.MOMENTUM_TYPE = 'bonus'               
        self.MOMENTUM_STRENGTH = 0.3 #.5              
        self.MOMENTUM_STEPS = 15                    
        ...
        # Load goal-specific RCNs if needed for EXPLOIT_LOCATIONS
        if mode == RobotMode.EXPLOIT_LOCATIONS and hasattr(self, 'active_goal_name'):
    ...
    def _load_or_init_pcn_for_scale(self, path, scale_def, num_grid_cells, enable_ojas, enable_stdp):
        try:
            ...
            if pcn_class_name == "MultiscalePlaceCellWithGridV8":
    ...
    def _initialize_new_pcn(self, scale_def, num_grid_cells, enable_ojas, enable_stdp):
        ...
        pcn = MultiscalePlaceCellWithGridV8(
            bvc_layer=bvc,
            ...
            try:
                ...
                for direction in range(self.n_hd):
                    if self.distances_per_hd[direction] < self.OBSTACLE_DISTANCE_THRESHOLD:
                        ...
                        best_reward = pcn.multi_step_preplay_constrained(
                            forced_first_direction=direction,
                            max_steps=self.MULTISTEP_STEPS,
                            debug=self.DEBUG_EXPLOIT_V1 and show_detailed_debug
                        )
    ...
    def exploit_v15(self):
        """
        Enhanced multiscale exploitation using multi-step preplay for gradient computation.
        
        Key improvements over v1:
        - Multi-step preplay on ALL scales for gradient computation
        - Direct scale weighting based on gradient quality
        - Noise filtering using MULTISTEP_THRESHOLD
        - No arbitrary scale selection - proportional weighting only
        """
        # Debug setup
        ...
        if show_debug:
            if not hasattr(self, '_exploit_v15_call_count'):
                self._exploit_v15_call_count = 0
            self._exploit_v15_call_count += 1
            
            show_detailed_debug = self._exploit_v15_call_count % 5 == 0
            if show_detailed_debug:
                print(f"\n{'='*60}")
                print(f"EXPLOIT V15 - MULTI-STEP GRADIENT-BASED - STEP {self._exploit_v15_call_count}")
                print("="*60)
        ...
        # 3) Multi-step preplay on ALL scales with error handling
        all_scale_rewards = []
        all_scale_gradients = []
        ...
        for scale_idx, scale_def in enumerate(self.scales):
            try:
                if show_detailed_debug:
                    print(f"Computing multi-step rewards for scale {scale_idx} ({scale_def['name']})")
                
                multi_step_rewards = self.compute_multi_step_rewards_all_directions(scale_idx)
                
                # Apply threshold to filter noise (treat values below MULTISTEP_THRESHOLD as zero)
                ...
                if max_reward > 1e-6:  # Avoid division by near-zero
                    normalized_rewards = thresholded_rewards / max_reward
                else:
                    normalized_rewards = thresholded_rewards  # Keep zeros as zeros
                ...
                if show_detailed_debug:
                    print(f"  Scale {scale_idx}: max_reward={max_reward:.3f}, gradient={gradient:.3f}")
        ...
        # 4) Handle all-zero gradients case
        ...
        # 6) Combine rewards across scales
        combined_pot_rew = torch.zeros(self.n_hd, dtype=self.dtype, device=self.device)
        for direction in range(self.n_hd):
            combined_pot_rew[direction] = sum(
        ...
        # 7) Apply momentum to combined rewards
        momentum_adjusted_rewards = self.apply_directional_momentum(combined_pot_rew)

        # 8) Compute action heading using circular mean
        angles = torch.linspace(0, 2 * np.pi * (1 - 1 / self.n_hd), self.n_hd, device=self.device, dtype=self.dtype)
        sin_component = torch.sum(torch.sin(angles) * momentum_adjusted_rewards)
        cos_component = torch.sum(torch.cos(angles) * momentum_adjusted_rewards)
        action_angle = torch.atan2(sin_component, cos_component)
        ...
        # 9) Determine chosen direction for momentum tracking
        chosen_direction = torch.argmax(momentum_adjusted_rewards).item()
        chosen_direction_reward = momentum_adjusted_rewards[chosen_direction].item()
    ...
    def compute_multi_step_rewards_all_directions(self, scale_idx, decay_factor=0.6):
        ...
        Returns:
            torch.Tensor: Multi-step weighted rewards for each direction [8]
        """
        pcn, rcn = self.pcns[scale_idx], self.rcns[scale_idx]
        multi_step_rewards = torch.zeros(self.n_hd, dtype=self.dtype, device=self.device)
        ...
        try:
            ...
            for direction in range(self.n_hd):
                if self.distances_per_hd[direction] < self.OBSTACLE_DISTANCE_THRESHOLD:
                    ...
                    best_reward = pcn.multi_step_preplay_constrained_weighted(
                        forced_first_direction=direction,
                        max_steps=self.MULTISTEP_STEPS,
                        decay_factor=decay_factor,
                        debug=False  # Avoid excessive debug output
                    )
                    multi_step_rewards[direction] = best_reward
