...
This module handles:
1. Random spawn location generation with constraints
2. A* pathfinding on discretized grids
3. Visualization for validation
...
class PathPlanner:
    ...
    def __init__(self, world_name: str, wall_clearance: float = 0.2):
        ...
        self.world_config = get_world_config(world_name)
    ...
    def _mark_rectangle_obstacle(self, grid: np.ndarray, obstacle: Dict[str, Any]):
        ...
        """
        corners = np.asarray(expanded_obstacle_corners(obstacle, self.wall_clearance), dtype=np.float64)
        ...
        xs = np.arange(grid_min_x, grid_max_x + 1) * self.grid_cell_size - half_width + self.grid_cell_size / 2
        ys = -(np.arange(grid_min_y, grid_max_y + 1) * self.grid_cell_size - half_height + self.grid_cell_size / 2)
        local_x, local_y = np.meshgrid(xs, ys)
        inside = obstacle_contains_points(local_x, local_y, obstacle, self.wall_clearance)

        # Mark obstacle cells using the true footprint, not just its AABB.
        grid[grid_min_y:grid_max_y+1, grid_min_x:grid_max_x+1] |= inside
...
def is_within_obstacle(pos: List[float], obstacle: Dict[str, Any], wall_clearance: float) -> bool:
    ...
    """
    if obstacle["type"] == "rectangle":
        return obstacle_contains_points(pos[0], pos[1], obstacle, wall_clearance)
...
def generate_spawn_locations(world_name: str, goals: List[Dict], trials_per_goal: int,
    ...
    """
    world_config = get_world_config(world_name)
    world_size = world_config["size"]
...
def visualize_trial_setup(world_name: str, start_pos: List[float], goal_info: Dict,
    ...
    # Draw obstacles using their true footprint. Bounds are still retained in
    # parsed configs for broad-phase checks and legacy consumers.
    rectangle_obstacles = [
        obstacle for obstacle in planner.world_config["obstacles"]
        if obstacle["type"] == "rectangle"
    ]
    for obstacle_idx, obstacle in enumerate(rectangle_obstacles):
        if obstacle["type"] == "rectangle":
            corners = np.asarray(expanded_obstacle_corners(obstacle), dtype=np.float64)
            orig_poly = patches.Polygon(
                corners,
                facecolor='red', alpha=0.7, edgecolor='darkred',
                label='Obstacle (Original)' if obstacle_idx == 0 else ""
            )
            ax.add_patch(orig_poly)

            # Obstacle with clearance (lighter red)
            clearance_corners = np.asarray(
                expanded_obstacle_corners(obstacle, wall_clearance),
                dtype=np.float64,
            )
            expanded_poly = patches.Polygon(
                clearance_corners,
                facecolor='red', alpha=0.3, edgecolor='red', linestyle='--',
                label='Obstacle + Clearance' if obstacle_idx == 0 else ""
            )
            ax.add_patch(expanded_poly)
