...
class PlaceCellLayer:
    ...
    def __init__(self, num_pc=1000, input_dim=720, timestep=32*3, max_dist=12, n_hd=8):
        ...
        self.bvcLayer = BoundaryVectorCellLayer(
            max_dist=max_dist, input_dim=input_dim, n_hd=n_hd, sigma_ang=90, sigma_d=0.5
        )
    ...
    def get_place_cell_activations(self, input_data, hd_activations, mode="learn", collided=False):
        ...
        self.activation_update += self.tau_p * (
            -self.activation_update + afferent_excitation - afferent_inhibition - recurrent_inhibition
        )
        ...
        if np.any(self.place_cell_activations) and mode == "dmtp" and not collided:
            ...
            self.w_rec_hd_place += tf.cast(
                ...
                tf.tensordot(
                    self.place_cell_activations[:, np.newaxis], self.place_cell_trace[np.newaxis, :], axes=1
                ) -
                tf.tensordot(
                    self.place_cell_trace[:, np.newaxis], self.place_cell_activations[np.newaxis, :], axes=1
                )
            )
    ...
    def exploit(self, direction, num_steps=1):
        ...
        for step in range(num_steps):
            ...
            place_cell_activations = tf.tanh(
                tf.nn.relu(
                    tf.tensordot(
                        tf.cast(self.w_rec_hd_place[direction], tf.float32), previous_activations, axes=1
