...
def generate_place_cells_report(
    ...
    def generate_vibrant_colors(n):
        colors_rgb = []
        for i in range(n):
            # Use HSV color space for more vibranResult
            # t colors
            hue = i / n
...
if __name__ == "__main__":
    ...
    # Load hmap data from visualization/pkl/<world_name>/hmaps
    base_dir = Path(__file__).resolve().parent / "pkl" / WORLD_NAME / "hmaps"
    hmap_loc = pickle.load(open(base_dir / "hmap_loc.pkl", "rb"))
    hmap_pcn = pickle.load(open(base_dir / "hmap_pcn.pkl", "rb"))
