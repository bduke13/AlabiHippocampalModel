...
def load_hmaps(hmap_names, world_name):
    """
    Load one or more history map pickle files from visualization/pkl/<world_name>/hmaps.
    """
    base_dir = Path(__file__).resolve().parent / "pkl" / world_name / "hmaps"
    hmaps = {}
    for name in hmap_names:
        file_path = base_dir / f"{name}.pkl"
        with open(file_path, "rb") as f:
            hmaps[name] = pickle.load(f)
    if len(hmap_names) == 1:
        return hmaps[hmap_names[0]]
    return hmaps
